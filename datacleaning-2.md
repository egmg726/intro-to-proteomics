...
y.peptide <- readDIANN(file='data/report_stool.parquet',format="parquet")
```

``` error
Error in readDIANN(file = "data/report_stool.parquet", format = "parquet"): arrow package required but is not installed (or can't be loaded)
```
